 ...
 * @author Admin
 */
public class newQuizLesson extends HttpServlet {
...
//        processRequest(request, response);
        ...
        if (courseId!=null && !courseId.equals("default")) {
            ...
            request.getRequestDispatcher("newQuizLesson.jsp").forward(request, response);
            return;
        }
...
//        processRequest(request, response);
        ...
        if(mode!=null && mode.equals("edit")){
            ...
            response.sendRedirect("listQuizLesson?editBlogStatus=Edit Quiz successfully!");
            return;
        }
        ...
        String correctAnswer = request.getParameter("correctAnswer");
        if (!lessonId.equals("default") && !correctAnswer.equals("default")) {
            //Add to DB
            String question = request.getParameter("question");
            String explain = request.getParameter("explain");
            String answer[] = request.getParameterValues("answer");
            ...
            q.addQuestion(size + 1 + "", question, explain, lessonId);
            a.addAnswer(answer, (size + 1) + "");
            ca.addCorrectAnswer(answer[Integer.parseInt(correctAnswer)-1], (size+1)+"");
            response.sendRedirect("listQuizLesson?editBlogStatus=New Quiz successfully!");
        } else {
            request.setAttribute("listCourses", listCourses);
